...
<div class="input-group" id="ig__password">
    <input type="password" autocomplete="new-password" data-password-strength="pst__password" data-table="users" data-field="x__password" name="x__password" id="x__password" value="<?= $Page->_password->EditValue ?>" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
...
<span id="el_users_role">
    <select
        ...
        data-select2-id="fusersedit_x_role"
        data-table="users"
        data-field="x_role"
...
loadjs.ready("fusersedit", function() {
    var options = { name: "x_role", selectId: "fusersedit_x_role" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersedit.lists.role.lookupOptions.length) {
        options.data = { id: "x_role", form: "fusersedit" };
    } else {
        options.ajax = { id: "x_role", form: "fusersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
