...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fsales_order_detailadd.lists.product_id = <?= $Page->product_id->toClientList($Page) ?>;
    fsales_order_detailadd.lists.unit_price = <?= $Page->unit_price->toClientList($Page) ?>;
...
<span id="el_sales_order_detail_product_id">
    <select
        ...
        data-select2-id="fsales_order_detailadd_x_product_id"
...
<script>
loadjs.ready("fsales_order_detailadd", function() {
    var options = { name: "x_product_id", selectId: "fsales_order_detailadd_x_product_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsales_order_detailadd.lists.product_id.lookupOptions.length) {
        options.data = { id: "x_product_id", form: "fsales_order_detailadd" };
    } else {
        options.ajax = { id: "x_product_id", form: "fsales_order_detailadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fsales_order_detailadd", function() {
    fsales_order_detailadd.createAutoSuggest(Object.assign({"id":"x_unit_price","forceSelect":false}, ew.vars.tables.sales_order_detail.fields.unit_price.autoSuggestOptions));
});
</script>
